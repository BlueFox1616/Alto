...
public class Space: Identifiable, Equatable {
    ...
    public var index: Int? {
        if let index = AltoData.shared.spaces.firstIndex(where: { $0.id == self.id }) {
            return index
        }
        return nil
    }
    ...
    public var currentTab: ADKTab? // maybe make this computed in the future
